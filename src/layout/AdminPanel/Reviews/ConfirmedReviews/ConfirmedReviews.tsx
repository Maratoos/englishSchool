...
export const ConfirmedReviews: FC = () => {
  ...
  const { documents, error, isPending } = getCollection<IReview>("confirmedReviews")
  ...
  return (
    <>
      <Typography text="Отзывы показанные на странице:" margin="50px 0 50px 0" />
            ...
            {!isPending &&
              ...
              documents.map((doc) => (
                <TableRow key={doc.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                  ...
                  <TableCell sx={{ padding: "8px" }} align="right" width={150}>
                    <Button onClick={() => handleModal(doc)} variant="contained">
                      Изменить
                    </Button>
                  </TableCell>
                </TableRow>
